...
function density_default() {
  var x3 = defaultX, y3 = defaultY, weight = defaultWeight, dx = 960, dy = 500, r = 20, k = 2, o = r * 3, n = dx + o * 2 >> k, m2 = dy + o * 2 >> k, threshold2 = constant_default5(20);
  function grid(data) {
      ...
      *neighbors(i) {
      ...
      _clipSegment(x06, y06, x12, y12, c0, c1) {
        ...
        while (true) {
          ...
          let x3, y3, c2 = c0 || c1;
          if (c2 & 8) x3 = x06 + (x12 - x06) * (this.ymax - y06) / (y12 - y06), y3 = this.ymax;
          else if (c2 & 4) x3 = x06 + (x12 - x06) * (this.ymin - y06) / (y12 - y06), y3 = this.ymin;
          else if (c2 & 2) y3 = y06 + (y12 - y06) * (this.xmax - x06) / (x12 - x06), x3 = this.xmax;
      ...
      _project(x06, y06, vx, vy) {
        ...
        if (vy < 0) {
          ...
          if ((c2 = (this.ymin - y06) / vy) < t) y3 = this.ymin, x3 = x06 + (t = c2) * vx;
        } else if (vy > 0) {
          if (y06 >= this.ymax) return null;
          if ((c2 = (this.ymax - y06) / vy) < t) y3 = this.ymax, x3 = x06 + (t = c2) * vx;
        }
        if (vx > 0) {
          if (x06 >= this.xmax) return null;
          if ((c2 = (this.xmax - x06) / vx) < t) x3 = this.xmax, y3 = y06 + (t = c2) * vy;
        } else if (vx < 0) {
          if (x06 <= this.xmin) return null;
          if ((c2 = (this.xmin - x06) / vx) < t) x3 = this.xmin, y3 = y06 + (t = c2) * vy;
        }
        return [x3, y3];
      }
      _edgecode(x3, y3) {
    ...
    Delaunay = class _Delaunay {
      ...
      *neighbors(i) {
      ...
      find(x3, y3, i = 0) {
        ...
        const i0 = i;
        let c2;
        while ((c2 = this._step(i, x3, y3)) >= 0 && c2 !== i && c2 !== i0) i = c2;
        return c2;
      }
      _step(i, x3, y3) {
...
function link_default(links) {
  var id2 = index2, strength = defaultStrength, strengths, distance = constant_default6(30), distances, nodes, count2, bias, random, iterations2 = 1;
  if (links == null) links = [];
...
function centroidLinePoint(lambda, phi) {
  lambda *= radians, phi *= radians;
  var cosPhi = cos2(phi), x3 = cosPhi * cos2(lambda), y3 = cosPhi * sin2(lambda), z = sin2(phi), w = atan2(sqrt2((w = y0 * z - z0 * y3) * w + (w = z0 * x3 - x0 * z) * w + (w = x0 * y3 - y0 * x3) * w), x0 * x3 + y0 * y3 + z0 * z);
  W1 += w;
  X1 += w * (x0 + (x0 = x3));
  Y1 += w * (y0 + (y0 = y3));
  Z1 += w * (z0 + (z0 = z));
...
function polygonContains_default(polygon, point2) {
  ...
  var sum3 = new Adder();
  if (sinPhi === 1) phi = halfPi3 + epsilon5;
  else if (sinPhi === -1) phi = -halfPi3 - epsilon5;
  ...
  function clipLine(stream) {
    ...
    return {
      ...
      point: function(lambda, phi) {
        var point1 = [lambda, phi], point2, v2 = visible(lambda, phi), c2 = smallRadius ? v2 ? 0 : code(lambda, phi) : v2 ? code(lambda + (lambda < 0 ? pi3 : -pi3), phi) : 0;
...
function resample(project, delta2) {
  function resampleLineTo(x06, y06, lambda04, a0, b02, c0, x12, y12, lambda12, a1, b12, c1, depth, stream) {
    ...
    if (d2 > 4 * delta2 && depth--) {
      var a2 = a0 + a1, b = b02 + b12, c2 = c0 + c1, m2 = sqrt2(a2 * a2 + b * b + c2 * c2), phi2 = asin(c2 /= m2), lambda22 = abs3(abs3(c2) - 1) < epsilon5 || abs3(lambda04 - lambda12) < epsilon5 ? (lambda04 + lambda12) / 2 : atan2(b, a2), p = project(lambda22, phi2), x22 = p[0], y22 = p[1], dx2 = x22 - x06, dy2 = y22 - y06, dz = dy * dx2 - dx * dy2;
...
function projectionMutator(projectAt) {
  var project, k = 150, x3 = 480, y3 = 250, lambda = 0, phi = 0, deltaLambda = 0, deltaPhi = 0, deltaGamma = 0, rotate, alpha = 0, sx = 1, sy = 1, theta = null, preclip = antimeridian_default, x06 = null, y06, x12, y12, postclip = identity_default2, delta2 = 0.5, projectResample, projectTransform, projectRotateTransform, cache, cacheStream;
...
function azimuthalInvert(angle2) {
  return function(x3, y3) {
    var z = sqrt2(x3 * x3 + y3 * y3), c2 = angle2(z), sc = sin2(c2), cc2 = cos2(c2);
    return [
      atan2(x3 * sc, z * cc2),
      asin(z && y3 * sc / z)
    ];
...
function mercatorRaw(lambda, phi) {
  return [lambda, log2(tan((halfPi3 + phi) / 2))];
}
...
function mercatorProjection(project) {
  ...
  function reclip() {
    var k = pi3 * scale2(), t = m2(rotation_default(m2.rotate()).invert([0, 0]));
...
function transverseMercatorRaw(lambda, phi) {
  return [log2(tan((halfPi3 + phi) / 2)), -lambda];
}
    ...
    gamma_default = function sourceRandomGamma(source) {
      ...
      function randomGamma(k, theta) {
        ...
        return function() {
          do {
            do {
              var x3 = randomNormal(), v2 = 1 + c2 * x3;
            } while (v2 <= 0);
            v2 *= v2 * v2;
            var u4 = 1 - source();
  ...
  "node_modules/react-gauge-chart/dist/GaugeChart/index.js"(exports) {
    ...
    var GaugeChart = function GaugeChart2(initialProps) {
      ...
      (0, _customHooks.default)(function() {
        if (props.nrOfLevels || prevProps.current.arcsLength.every(function(a2) {
          return props.arcsLength.includes(a2);
        }) || prevProps.current.colors.every(function(a2) {
          return props.colors.includes(a2);
        })) {
          (0, _utils.setArcData)(props, nbArcsToDisplay, colorArray, arcData);
        }
